...
To make the definition in \autoref{sec:methodology-language-design:data-format} make sense we need to define mathematical semantics. This will be done in three levels of abstraction:
\begin{enumerate}
...
\begin{itemize}
    ...
    \item `travel` $\textit{trav}: T \times T \longrightarrow \N_0 \cup \{ \infty \}$\todo{Properly define this}
\end{itemize}
...
\begin{itemize}
    ...
    \item For every task we need the cost function defined in (\nameref{subsec:formal-framework:semantics:computing-cost}) to be finite.\\
          $c(a) \in \N_0$
    \item $\forall t_1, t_2 \in T_s | (a(t_1) < a(t_2) \land \forall t \in T_s: a(t) > a(t_1) \implies a(t) > a(t_2)): \textit{trav}(t_1, t_2) \in N_0$ \todo{Properly define this}\\
...
\end{align*}
The $\delta$ function above is defined as the Kronecker delta \cite{bib:2007-kozen}.
%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%